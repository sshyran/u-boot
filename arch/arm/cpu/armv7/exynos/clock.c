...
static struct clk_bit_info exynos5420_bit_info_table[] = {
	...
	{PERIPH_ID_I2C10, -1, 0x10504, -1, -1, 8, -1, -1, 0x3f, -1},
...
static int exynos_get_pll_clk(int pllreg, unsigned int r, unsigned int k)
{
	...
	/*
	 * Certain registers use 10 bit wide field to store mdiv value.
	 * The rest use 9 bits.
	 */
	switch (pllreg) {
	case APLL:
...
static unsigned long exynos542x_get_lcd_clk(void)
{
	struct exynos5420_clock *clk =
		(struct exynos5420_clock *)samsung_get_base_clock();
	unsigned long sclk;
	...
	sel = (readl(&clk->clk_src_disp10) >> 4) & 7;

	if (proid_is_exynos5422()) {
		/*
		 * Mapping of CLK_SRC_DISP10 CLKMUX_FIMD1 [6:4] values into
		 * PLLs. The first element is a placeholrer to bypass the
		 * default settig.
		 */
	 ...
	 * FIMD1_RATIO [3:0]
	 */
	ratio = readl(&clk->clk_div_disp10) & 0xf;

	return sclk / (ratio + 1);
}

static void exynos4_set_lcd_clk(void)
{
	struct exynos4_clock *clk =
	    (struct exynos4_clock *)samsung_get_base_clock();
	unsigned int cfg = 0;
...
static void exynos5_set_lcd_clk(void)
{
	struct exynos5_clock *clk =
	    (struct exynos5_clock *)samsung_get_base_clock();
	unsigned int cfg = 0;
...
static void exynos542x_set_lcd_clk(void)
{
	...
	if (proid_is_exynos5422()) {
		 ...
		 * 111: SCLK_RPLL
		 */
		cfg = readl(&clk->clk_src_disp10) | (7 << 4);
		writel(cfg, &clk->clk_src_disp10);
	}
#endif
}
